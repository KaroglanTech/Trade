...
public class Translator {
    ...
    public Translator(Trade plugin) throws IOException {
        logger = plugin.getLogger();
        assetManager = Sponge.getAssetManager();
        lang = plugin.getConfigNode().getNode("Trade").getNode("Language").getString();
        ...
        if (!(langFile = new File(langPath + lang + ".properties")).exists()) {
            if (!assetOpt.isPresent()) {
                ...
                logger.info("https://github.com/Tollainmear/Trade/tree/master/resources/assets/karoglansigneditor/lang");
                plugin.getConfigNode().getNode("Trade").getNode("Language").setValue("en_US");
                ...
                if (!assetOpt.isPresent()) {
                    ...
                    logger.warn("https://github.com/Tollainmear/Trade/issues");
                    return;
                }
